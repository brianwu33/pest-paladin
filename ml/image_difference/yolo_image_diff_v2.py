...
parser.add_argument("--min_diff_size", type=int, default=500, help="Minimum difference size (w*h)")
...
parser.add_argument("--bg_method", type=str, choices=["baseline", "mog2", "knn"], default="baseline", help="Background subtraction method")
...
# ------------------------
# Capture Setup and Frame Timing
# ------------------------
...
while True:
    ...
    for bbox in merged_boxes:
        ...
        # Draw ROI on the original frame and run YOLO detection on it
        x, y, w, h = expanded_bbox
        cv.rectangle(original_frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
        run_yolo_on_region(original_frame, expanded_bbox)
