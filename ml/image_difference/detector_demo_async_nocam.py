...
# ------------------------
# cap = cv.VideoCapture(0)  # Non-Mac: Use default webcam (change to 1 if using an external camera)
cap = cv.VideoCapture(0, cv.CAP_AVFOUNDATION) # On Mac: iPhone
# cap = cv.VideoCapture(1, cv.CAP_AVFOUNDATION) # On Mac: Web Cam
...
if args.bg_method == "baseline":
    # Capture a single frame to initialize the background
    ret, frame = cap.read()
    if not ret:
        print("Error: Failed to capture frame for background initialization.")
        cap.release()
        exit()
...
delay_ms = int(1000.0 / args.live_fps) if live_mode else 1  # Control frame rate
